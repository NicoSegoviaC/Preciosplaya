...
def update_figure(selected_especie, nivel, region):
    if nivel == 'Nivel nacional':
        ...
        df_especie = df_last_10_years[(df_last_10_years["Especie"] == selected_especie) & (df_last_10_years["region"] == region)]

    if df_especie.empty:
        return go.Figure(layout=go.Layout(title="Sin registros de precio playa para el rango solicitado"))
    ...
    fig.update_layout(
        ...
        annotations=[
            #dict(text="Datacenter SECOS", xref="paper", yref="paper", x=1, y=0.98, showarrow=False, xanchor="right", yanchor="bottom", font=dict(size=8)),
            dict(text="Fuente: Sernapesca", xref="paper", yref="paper", x=1, y=0.95, showarrow=False, xanchor="right", yanchor="bottom", font=dict(size=8))
        ]
    )
    return fig
